...
class FlinkScalaExcelTableSinkIntegrationSpec extends FlatSpec with BeforeAndAfterAll with GivenWhenThen with Matchers {
  ...
  "The file testsimple.xlsx" should "be read as a tablesource with header, written using tablesink with header and re-read with header" in {
    ...
    When("read as a tablesource in local Flink environment, write back using tablesink and reread of written data")
